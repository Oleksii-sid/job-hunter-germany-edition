        ...
        try:
            ...
            for card in job_cards[:10]:
                try:
                    ...
                    if not href:
                        continue
                    full_url = urllib.parse.urljoin("https://www.stepstone.de", href)
        ...
        try:
            ...
            for card in cards[:15]:
                try:
                    ...
                    if not href:
                        continue
                    full_url = urllib.parse.urljoin(site_data.career_url, href)
                    results.append({"title": title, "url": full_url})
                except Exception:
                    continue
        finally:
...
def search_indeed_selenium(query: str, location: str = "Berlin", limit: int = 5):
    ...
    # ⚠️ В Docker ставим True, локально False
    ...
    try:
        ...
        driver.get(url)
        time.sleep(15)  # ждём загрузку / капчу
        ...
        for card in cards[:limit]:
            ...
            jobs.append({"title": title_el.get_text(strip=True), "url": link})
